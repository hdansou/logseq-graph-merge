...
To learn _how_ the merge decides things, read [merge-workflow.md](merge-workflow.md). For _why_, read [requirements.md](requirements.md).
...
| Java                            | Needed by babashka's dependency resolution _(not verified: this machine already had Java)_       | `java -version`                    |
...
**Use exact graph names.** Logseq CLI commands silently _create_ a graph that doesn't exist. The merge tool checks `graph list` before touching anything, but be careful when running `logseq` commands by hand.
...
| `… failed: … server-start-timeout-orphan` or `… failed to publish health` for one source | That build couldn't start a server for the graph. The real error is in `~/logseq/graphs/<graph>/db-worker-node-<date>.log`. Seen for `Library-Test` on the upcoming build (`Cannot store nil as a value at {:db/id nil, :block/tx-id …}`, requirements trap 17). That graph was repaired on 2026-09-18 and opens on every build; if another graph shows it, see [library-test-investigation.md](library-test-investigation.md) for the cause and the index-rebuild repair. |
...
| `Merge failed:` anything after `Imported into <dest>`                                    | The destination graph already exists and is **half-built**, so a re-run stops at `Destination graph "…" already exists`. Remove it with `logseq graph remove <dest>`, then re-run. Nothing in the sources was touched.                                                                                                                          |
